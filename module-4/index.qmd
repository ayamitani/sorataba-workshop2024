...
* Requires correct specification of the dropout model ($\Pr(R_{ij}=1|R_{i1}=\cdots R_{i,j-1}, X_{i}, Y_{i1},...,Y_{i,j-1})$)
...
$$
$$
D_{i}=1+\sum_{j=1}^{n}R_{ij}
$$
For example,

* an individual who completes the study with no missed visits will have $Y_{i} = (Y_{i1},...,Y_{in})^{T}$ and $D_{i}=n+1$
* an individual who drops out after the third visit will have $Y^{O}_{i} = (Y_{i1},Y_{i2},Y_{i3})^{T}$ and $D_{i}=3+1=4$
