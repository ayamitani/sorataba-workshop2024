...
## Model mean for dental study{.smaller}
...
dentallong <- dental %>%
  ...
  group_by(id) %>%
  mutate(time = row_number(),
         # for this model, we want age as continuous
         age = as.numeric(age), 
         # make a 1/0 binary variable group
         group = case_when(     
           gender == "F" ~ 0,
           gender == "M" ~ 1
         ),
         # code to apply truncated linear function
         ageknot = (age - knot) * I(age >= knot)) %>% 
