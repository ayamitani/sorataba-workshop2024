...
* We can introduce more flexibility by explicitly by modeling the correlation structure in $\small R_{i}$
  - i.e. Move beyond $\small  R_{i} = \sigma^{2}I_{n_{i}}$ and permit the $\small \epsilon_{i}$ to be correlated
* The hybrid random intercepts and covariance pattern model looks like:
$$
Y_{ij} = X^{T}_{ij}\beta + b_{0i} + U_{i}(t_{ij}) + \epsilon^{*}_{ij},
$$
where
  - $\small b_{0i}$ is an individual-specific random intercept with $\small E(b_{0i})=0$ and $\small \text{Var}(b_{0i}) = \sigma^{2}_{b}$
  - $\small U_{i}(t_{ij})$ is a covariance pattern (serial dependence) term 
  - We assume $\small E(\epsilon^{*}_{ij})=0$ and $\small \text{Var}(\epsilon^{*}_{ij}) = \sigma^{2}$
  - Finally, we assume $\small U_{i}(t_{ij})$ are assumed to have a normal distribution, with zero mean, variance $\small \sigma^{2}_{u}$, and correlation
$$
\text{Corr}\left\{U_{i}(t_{ij}),U_{i}(t_{ik})\right\} = \rho(|t_{ij}-t_{ik}|).
$$


* Note that $\small \epsilon^{*}_{ij}$ captures the variation in error terms beyond that is explained by $\small U_{i}(t_{ij})$
